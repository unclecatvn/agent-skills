...
- Keep printable report actions and paper formats in `<model>_reports.xml`
  and QWeb report templates in `<model>_templates.xml`.
- Keep SQL-view report Python and backend views together as
  `<model>_report.py` and `<model>_report_views.xml`.
- Use stable, descriptive XML IDs and record names.
